...
def test_get_blog_entry_by_slug(django_assert_num_queries):
    ...
    blog_entry = baker.make("web.BlogEntry", is_draft=False, category=blog_category)

    with django_assert_num_queries(1):
        result = blog_entry_providers.get_blog_entry_by_slug(
            category_slug=blog_entry.category.slug, slug=blog_entry.slug
        )
